...
**
** parameters:			timer number: 0-3
** Returned value:		None
**
******************************************************************************/
void enable_timer( uint8_t timer_num ) //abilita il timer corrispondente al numero passato; da eseguire per forza per far funzionare il timer
{
  /*if ( timer_num == 0 )
  {
	  LPC_TIM0->TCR = 1;
...
uint32_t init_timer ( uint8_t timer_num, uint32_t TimerInterval ) //inizializzazione del timer indicato al valore fornito
{
  if (timer_num == 0){
...
//	 <i> 0 Feature disabled.
//   </e> */
	  LPC_TIM0->MCR = 7; //valore numerico corrispondente alla rappresentazione binaria usata per impostare stop, interrupt e reset del timer spiegati nel blocco di commenti sopra
		//tre bit a/b/c
		//a (più significativo) => stop del timer una volta raggiunto il valore se settato a 1
		//b => reset del timer counter una volta raggiunto il valore se settato a 1
		//c (meno significativo) => scatena un interrupt una volta raggiunto il valore se settato a 1
// </h>
